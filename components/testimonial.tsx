...
const TestimonialCard = ({ profileImage, name, feedback, mainImage }: (typeof testimonials)[0]) => (
  <div className="group cursor-pointer">
    <div className="relative rounded-lg overflow-hidden mb-4 bg-gray-200 dark:bg-gray-800">
      <img
        src={mainImage}
        alt={feedback}
        className="w-full h-auto object-cover transition-all duration-700 group-hover:scale-105"
        onError={(e) => { e.currentTarget.src = 'https://placehold.co/800x600/1a1a1a/ffffff?text=Image'; }}
      />
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <img
          ...
          alt={name}
          onError={(e) => { e.currentTarget.src = 'https://placehold.co/40x40/EFEFEF/333333?text=A'; }}
        />
        <p className="text-xs uppercase tracking-widest text-gray-400 dark:text-gray-300 font-light">
          {name}
        </p>
      </div>
      <p className="text-sm text-gray-600 dark:text-gray-300 font-light leading-relaxed group-hover:text-black dark:group-hover:text-white transition-colors">
        {feedback}
      </p>
    </div>
  </div>
